...
class REST_Controller extends CI_Controller {
	...
	public function _remap($object_called, $arguments)
	{
		...
		$controller_method = $object_called . '_post';
	...
	public function response($data = array(), $http_code = null)
	{
		...
		else
		{
			...
			elseif (method_exists($this->format, 'to_'.$this->response->format))
			{
				// Set the correct format header
				header('Content-Type: '.$this->_supported_formats[$this->response->format]);
				$output = $this->format->factory($data)->{'to_'.$this->response->format}();// commented by rakesh
				//$output = $data;
			}
	...
	{
		//log_info('| uri '.$this->uri->uri_string().' |  method '.$this->request->method.' | params '.serialize($this->_args).' | ip_address '.$this->input->ip_address().' | time '.date('Y:m:d:h:m:s'));
	}
